...
// Electric shape). Replaces the repo-file `.exponential/config.json` targets:
// a run config is just a named argv the desktop apps spawn into a terminal
// tab. SECURITY: because this is DB-stored argv executed locally, desktops
// MUST keep the per-device Trust & Run commandSetHash prompt and re-hash
// whenever the fetched config set changes — never auto-run synced values.
// `workspace_id` is denormalized from the project server-side on insert
// (tRPC-only writes, so no trigger needed).
export const runConfigs = pgTable(
  `run_configs`,
  {
    ...
    // (same rule as the legacy PlatformCommon env).
    env: jsonb()
