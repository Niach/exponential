...
| `EXP_DEV_SCREEN` | `settings` \| `account` (= settings) \| `devices` \| `actions` \| `automations` \| `workflows` \| `workflow:<workflow-uuid>` \| `chat` \| `chat?issues=<a>,<b>&action=<id>&pr=<issue-uuid>&device=<id>&text=<text>` \| `reviews` \| `getting-started` \| **`board-issues`** \| **`inbox`** \| **`inbox-my-issues`** \| **`support`** \| **`files`** \| **`source-control`** \| `issue:<issue-uuid>` \| `pr:<issue-uuid>` \| `support:<thread-uuid>` \| `session:<coding_sessions row id>` | Pre-route the first screen. **EXP-851:** the rail's tool windows are SCREENS now — `board-issues` (also `board` / `issues`) is the active board's full-width list, `inbox` / `inbox-my-issues` the two tabs of the personal list, and `support` / `files` / `source-control` the remaining three. A board list takes the window's ACTIVE board, so `EXP_DEV_BOARD_ID` still picks which one. `chat` (EXP-825) is the Agent page — the composer with the Running/Past session rows stacked under it (EXP-851) — and its optional query mirrors the web route's one-shot search params: `issues` a csv of issue uuids (one chip each), `action` an action id or `builtin:…`, `pr` the issue whose open PR prefills the fix-conflicts `pr` input, `device`/`text` presets. `pr:` is keyed by the ISSUE whose linked PR the diff shows, `support:` by the support thread id. `reviews` (EXP-706) is the full-page Reviews list. `session:` (EXP-746) opens one coding session, keyed by the `coding_sessions` ROW id; with no engine in this process the screen dials the relay as a viewer, so it shows a Reconnecting state unless a machine is publishing that session (the `steering` capture pairs it with `bun run screenshots:desktop`). `workflows` / `workflow:` (EXP-981) are the Workflows list and one workflow's graph. Unset = the empty state. |
...
| `EXP_DEV_RAIL_HOT` | issue uuid | **New (EXP-998).** Photograph the blocks rail OPEN — the arrows to the left of the row dots, that issue's own edges lit — on the board list and the `ListNav`. At rest only the dots show; a pointer over a dot is what opens it in a real run. Ignored when the issue is not a visible row. |
| `EXP_DEV_RUN_FACE` | `diff` (also `changes`) \| `run` (also `transcript`) | **New (EXP-895).** Which FACE a `session:` screen opens on. `diff` opens the run's Changes face — the `Changes +N −M · K files · branch · state` bar over the file list and the per-file cards — instead of the transcript, so the diff is photographed without synthetic input. It only applies to a run that HAS changes: a session with no published diff and no PR/branch files renders the transcript anyway. Unset (and anything unrecognised) = the transcript. |
...
| `EXP_DEV_OPEN_SHELL` | `1` | Open one shell tab (a fullscreen terminal screen + its session-bar tab). |
...
| `EXP_DEV_AGENT_ACCOUNT` | email | Settings → Agents renders the LOCAL install's own sign-in and usage rows. With this set the pane never probes the machine: it stands demo accounts (claude + codex as this address) and pinned usage windows in place of the operator's real ones (EXP-698/733). The numbers match the shots stub's synced device row (`apps/web/scripts/screenshot-demo.ts` `DEMO_AGENT_STATUS`). EXP-951: the same demo payload is what `devices.register` and the heartbeat REPORT for this machine, so the lane sets it on EVERY launch — the capture host's row on the Agents screen never carries the operator's logins. |
