...
export interface DemoIds {
   ...
   * no such row, and the view skips rather than photographing the default
 ...
 * placeholder has no id — the caller skips that view with a note, which is the
 * honest outcome: a desktop launched with an unresolvable `EXP_DEV_SCREEN`
 * silently falls back to the default screen and would photograph the wrong view
