...
// Every distinct Lucide name the registry needs, deduped: an icon used by both
...
for (const name of allNames) geometry.set(name, loadIconNode(name))
...
import {
${allNames.map((n) => `  ${componentName(n)},`).join("\n")}
} from "lucide-react"
