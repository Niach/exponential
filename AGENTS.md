...
- **Marketing (`exponential.at`)** — Coolify app `exponential-marketing` (uuid `tcco4mztg1whf3yjmib2u9ph`). Coolify watches `master` and rebuilds via Nixpacks; base directory `/`, build `cd apps/marketing && bun run build`, start `npx -y serve apps/marketing/dist -l 80`. Auto-redeploys on push.
- **Push relay (`push.exponential.at`)** — Coolify app `exponential-push-relay` (uuid `u1299kfvca23s99opeizsnmh`). Coolify watches `master` and builds `Dockerfile.push-relay` (context `.`). Auto-redeploys on push.
