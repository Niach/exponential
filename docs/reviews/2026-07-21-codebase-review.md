...
## Addendum (2026-07-21, post-review master reconciliation)

Master moved while the findings were being filed (widget synthetic-user retirement / `issues.source`, EXP-2 proration flip, EXP-229 IDE logout on deploy, EXP-228 avatar follow-up). Reconciliation against those commits:

- **REV2-77 — cancelled.** The synthetic widget-bot user mechanism was removed entirely (migration `0039` deletes all `is_agent` users, drops `users.is_agent` + `widget_configs.widget_user_id`); there is no bot user left to orphan.
- **REV2-36 — scope reduced (medium → low, retitled).** Item 1 (the `issues.creator_id` cascade destroying teammates' work) was fixed on master (`ON DELETE SET NULL`). Item 2 stands: `attachments.uploader_id` still cascades, so account deletion leaves dangling image embeds in surviving issues — including the deleter's own now-surviving issues.
- **All other reviewed candidates stand unchanged** — verified against the merged tree: the account-deletion set (REV2-16, 37, 55, 75, 76), billing (REV2-30, 66, 70, 71, 86 — EXP-2 only flipped the proration constant), widget flow (REV2-25, 31, 69 — only creator/uploader nulling changed), comment-edit mentions (REV2-26 — `comments.update` still writes body only), missing indexes (REV2-8 — migration 0039 adds none), and the stale-comment items (REV2-98/99 — every quoted comment still present).
