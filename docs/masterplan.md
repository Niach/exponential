...
| 8 | Desktop settings | JetBrains-SDK-style: **Claude CLI path + workspace/repos path** (prefilled, editable) + branch prefix + personal API key (auto-minted; status row + Regenerate only). |
...
4. **User shell tabs (shipped on Linux).** The dock also hosts plain user shells, JetBrains-style: a `+` button in the dock header spawns the user's login shell (ghostty `command=null`) with cwd = the active repo's clone dir when cloned, else `$HOME`; titles auto-increment `Local`, `Local (2)`; shell tabs have no exit strip/run history and auto-close when the shell exits. A header-bar terminal button + shortcut toggles the dock even with zero tabs — expanding an empty dock immediately opens a fresh shell tab. macOS gets the same as part of the tabbed `MacTerminalDock` rework.
...
- [ ] **Desktop settings** (both): Claude CLI path, workspace/repos root, branch prefix (`exp/`), and a personal API key status row (the launcher auto-mints on demand via `users.mintPersonalApiKey`, name `Device: <hostname>`; Regenerate best-effort revokes ONLY this device's locally-stored old key id — never blanket-revoke, other devices hold their own keys); with a `claude --version` / `git --version` doctor.
...
- **Web** (owner/admin): new workspace-settings section `apps/web/src/components/workspace/repositories-section.tsx`, parallel to `projects-section.tsx` and registered in the settings nav. Lists workspace repos; "Connect repository" opens the **existing** `GithubRepoPicker` (`apps/web/src/components/github-repo-picker.tsx`) — which already handles the App-not-configured / not-installed / searchable-list states and the inline install flow — and calls `repositories.add`. A per-project link editor (multi-select of workspace repos with a primary star) drives `linkProject`/`setPrimary`. This section is also the **primary surface for GitHub App install state**: it shows a per-user install banner (install popup via `state=dialog` + window-focus re-detect, same convention as `GithubRepoPicker`) or a quiet "installed as `<account>` / Manage on GitHub" line from `integrations.github.status`; `/account/integrations` remains as the standalone page native apps deep-link to.
...
- Desktop settings pane (§4b), JetBrains-SDK-style, both desktops (Linux `settings.zig`, macOS `MacSettingsView.swift`): Claude CLI path (+ `claude --version` doctor), workspace/repos root, branch prefix (`exp/`), personal API key status row (auto-mint-on-demand via `users.mintPersonalApiKey`; per-device key names) — replacing any deleted `expk_` concept with the real user's key.
