...
| 4 Steer relay | 🟡 **relay service done + tested; macOS client WRITTEN (unverified); web router + Linux client + viewer UI missing** | DONE: `packages/steer-ticket` (HS256 tickets, 5 tests) + `apps/steer-relay` (Bun WS hub: device presence, session rooms keyed by coding_sessions.id, ring replay, single-steerer claim, slow-consumer eviction, admin endpoints /devices /sessions /start /kill; 15 tests incl. E2E) + Dockerfile.steer-relay + compose `--profile steer` + docs. ALSO IN-TREE: a complete **macOS steer client** (SteerApi, SteerProtocol, MacSteerControlChannel, MacSteerPublisher, MacSteerPtyTail + wiring in MacAppDependencies/MacCodingLauncher/MacCodingSettings/MacGhosttyTerminal) — never compiled (no Mac), dead code at runtime until the web router exists, known issues below. MISSING: the web `steer` tRPC router (REVERTED during the handoff scramble — re-derive from the spec below, do not hunt for it), the Linux client, all viewer UI (Phase 7), the Coolify relay app. Wire protocol FROZEN in `apps/steer-relay/src/protocol.ts` (+ a wire-truth pin added at masterplan §3.2). |
