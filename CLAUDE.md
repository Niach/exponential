...
- **GitHub App**: webhook Active (`${BETTER_AUTH_URL}/api/webhooks/github`, secret `GITHUB_WEBHOOK_SECRET`); `installation*` events arrive AUTOMATICALLY; only **Pull request** needs subscription. Claim flow: Callback URL `${BETTER_AUTH_URL}/api/integrations/github/callback`, client secret set, "Redirect on update" ticked, "Request user authorization during installation" UNCHECKED. The App HAS the `workflows` write permission. Gotchas: the installation owner must ACCEPT permission updates; cached installation tokens keep mint-time permissions ~1h. Staging + prod need separate Apps.
