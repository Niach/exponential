...
- **Desktop**: tag `desktop-v*` (or manual `workflow_dispatch`) triggers `.github/workflows/build-desktop.yml` — a codegen-drift guard (regenerates the committed Rust files and fails on diff) then unsigned macOS (arm64) + Linux (x86_64) release binaries as artifacts. Packaging/signing is manual (see checklist).
...
- **Desktop (Rust/gpui) distribution**: macOS `.app` needs a Developer ID cert, real `codesign`, and `xcrun notarytool submit`; Linux ships as AppImage/`.deb`/tarball; artifacts are built unsigned by `.github/workflows/build-desktop.yml` on `desktop-v*` tags — signing/notarization and packaging are manual release-time steps (TODO: no packaging pipeline yet, raw binaries only).
