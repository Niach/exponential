...
EXP-201; `coding/src/agent.rs`, contract `codingAgent`; savable default + per-agent path/model/effort in settings. **claude** (`--dangerously-skip-permissions`; plan mode: `--permission-mode plan --allow-dangerously-skip-permissions`), **codex** (always `--dangerously-bypass-approvals-and-sandbox`; MCP via `-c mcp_servers.*` argv, key rides spawn env `EXP_MCP_TOKEN` only), **pi** (no permission system/native MCP: the launcher writes a `.exp-pi-mcp.ts` extension loaded via `-e` bridging MCP over HTTP; `EXP_MCP_URL`/`EXP_MCP_TOKEN`). Ultracode is Claude-only; plan mode is claude+pi (pi via `.exp-pi-plan.ts`; remote-approvable plan card). The doctor probes all three but gates only the SELECTED agent (git always required); `devices.register` records the runnable agents so remote pickers offer only what the device runs (`steer.startSession` validates server-side; absent agent = claude). Resume relaunches the newest `runs.json` record for the subject (EXP-662); `.exp-agents` gates the native tail. EXP-484: `devices.agent_accounts`/`agent_usage`(+`_at`) jsonb = per-agent email/plan + rate-limit windows, collected READ-ONLY (`coding::agent_usage`, cache `agent-usage.json`, never a credential copy/refresh; ≤3×≤10), sent on heartbeat when changed; `coding_sessions.agent` keys the mobile Usage sheet to `agent_usage[agent]` (EXP-688: else the agent's Device-settings tab); command `agent_login` (cap `agent-login`) runs `claude auth login`/`codex login --device-auth` remotely, pi local only; Merge sits in the Latest-changes bar (diff vs merge-base of `origin/<default>`; IDE: above the bottom tabs).
