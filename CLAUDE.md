...
- **Desktop (Rust/gpui) distribution**: Linux AppImages and macOS `.app` bundles are now both built automatically by `build-desktop.yml` (both channels) — the macOS `.app` (`build-macos-app.sh`) is ad-hoc codesigned + zipped and declares the `exp://` scheme in its Info.plist, but still needs a Developer ID cert, real `codesign`, and `xcrun notarytool submit` for Gatekeeper-clean download distribution; Linux `.deb`/tarball not yet automated. URL-scheme registration is handled at runtime by the app's self-registration (Linux `.desktop`/`mimeapps.list`; macOS `LSSetDefaultHandlerForURLScheme`), not by a system installer.
