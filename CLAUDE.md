...
- **Issue lists (EXP-980)**: sub-issues nest under the parent, the ROOT decides group + position (`lib/issue-nesting.ts`); ONE blocks badge per row opens THE mini-graph (`lib/issue-graph.ts`; grid web + desktop, wave list phones; also `PrGraphBadge`, blocked-start dialog, workflow detail); ×4, fixture-locked. EXP-998: web md+ replaces the badge with the blocks RAIL (`lib/issue-rail.ts` pure lanes over the visible entries, `components/issue-rail.tsx` paints per row/header): at REST only the node dot at the row's right edge, hover reveals the arrows (`data-rail-open` on the list root, DOM-toggled), click opens the mini-graph; desktop mirrors it (`domain::issue_rail`, `crates/ui/src/issue_rail.rs` canvas per row/header, `EXP_DEV_RAIL_HOT` for captures); phones keep the badge; tree connectors = 3px corner, one unbroken tee vertical.
