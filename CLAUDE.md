...
Permissions are membership-only (`use-team-permissions.ts` + native mirrors); every member moderates and handles support. Signups get **no** team: first-run is create-or-join — "Create a team" (`teams.create`, open to every authed user; creator = owner; cloud free tier capped by `FREE_OWNED_TEAMS_CAP`) or "Join a team" (invite link; `teamInvites.accept` stamps `onboardingCompletedAt`). `teams.getDefault` is the NON-CREATING default-team resolver (oldest membership or null). An owner may delete ANY team incl. the last (cloud: only after its subscription is cancelled); invites optionally carry a synced `email` (server mails the link). The cloud's `feedback` team is ORDINARY since EXP-364 — every special-case guard (`getFeedbackTeamId`, undeletable, billing-cap exclusion) deleted, never reintroduce. **Billing (Creem) and the admin console are web-only.** **Desktop is the only client that runs coding sessions and publishes to the steer relay** (scrubbed activity channel only); since EXP-312 a LIVE session is visible/steerable ONLY by its owner — no view/steer perm split, no `perm`/`name` ticket claims; teammates see just the synced status badge.
...
GITHUB_POLLING                # 'true' = outbound merge cron (self-host behind NAT; decoupled from CLOUD_INSTANCE)
SES_WEBHOOK_SECRET            # SES bounce/complaint SNS webhook (unset = off)
CLOUD_INSTANCE                # opt-IN cloud marker (EXP-364, replaced SELF_HOSTED): 'true' = billing+limits+in-app widget; unset = self-hosted
FEEDBACK_WIDGET_KEY           # overrides the hardcoded cloud widget key (staging)
