...
The app is **noindex** everywhere: `__root.tsx` meta + ungated `X-Robots-Tag: noindex` from `server-bun.ts` (self-hosted too); `/robots.txt` allows crawling (`Disallow: /api/` only) so crawlers see it; no sitemap. Marketing owns the indexed surface (`sitemap.xml`/`llms.txt` from `PAGES` in `src/lib/seo.ts`).
