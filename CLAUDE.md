...
**Markdown:** `issues.description` + `comments.body` are plain `text` GFM — one interchange across web (TipTap + tiptap-markdown), iOS (cmark-gfm), desktop (comrak + the vendored WYSIWYG `crates/gpui-markdown-editor` [Apache-2.0 Velotype, see NOTICE]) and Android (`ui/markdown/`, commonmark-java). The round-trippable feature set IS `CONTRACT_FIXTURES`, byte-locked by desktop `wysiwyg_parity.rs` and the native mirrors — read the fixtures. **Underline intentionally unsupported**; tables, slash commands, image resize are out of scope. **Mentions** are plain `@<email>` — the server resolves members, fires `issue_mention` and auto-subscribes (`lib/integrations/mentions.ts`); clients render known members as pills and @-autocomplete inserts the plain form. **Issue mentions** are plain `#<IDENTIFIER>` (token contract `lib/issue-refs.ts`, per-client lookup, `#` autocomplete ×4); a pill renders only when it resolves to a synced same-team issue, and `resolveIssueRefs` fires NO notifications yet (deliberate). Images are stored relative `![alt](/api/attachments/{id})` — the server canonicalizes on save (`canonicalizeMarkdownImageUrls`), clients absolutize at fetch, attachments carry probed `width`/`height`. **Emoji** (EXP-551) are inserted as unicode, never `:shortcode:`; picker + `:` typeahead data is generated ONCE by `packages/emoji` into all four clients (drift-gated).
...
Workspace scripts: `bun --filter @exp/web <script>`; plain `cargo` from `apps/desktop/`. Generated `*.generated.rs` files are committed; regenerate only when their sources change. Do NOT run `bun run lint` — its --fix corrupts `typeof import()` sites (EXP-13).
...
**The operations runbook lives OUTSIDE the repo** (infra uuids/domains stay out of git; the operator's local agent memory). It covers infra uuids, buckets, staging, relay `TRUST_PROXY=true`, per-platform release steps/signing and the release checklist. Consult it before anything deploy-shaped; never re-inline it.
