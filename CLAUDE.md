...
The app is **noindex** everywhere: `__root.tsx` meta + ungated `X-Robots-Tag: noindex` from `server-bun.ts`; `/robots.txt` disallows only `/api/`; no sitemap. Marketing owns the indexed surface (`sitemap.xml`/`llms.txt` from `PAGES` in `src/lib/seo.ts`).
