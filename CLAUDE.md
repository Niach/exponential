...
Workspace scripts: `bun --filter @exp/web <script>` or `cd apps/web && bun run <script>`; plain `cargo` from `apps/desktop/`. Generated `*.generated.rs` files are committed; regenerate only when their sources change. Do NOT run `bun run lint` — its --fix corrupts `typeof import()` sites (EXP-13).
