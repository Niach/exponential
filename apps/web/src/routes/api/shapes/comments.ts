    ...
    handlers: {
      GET: createShapeRouteHandler({
        ...
        getWhere: async (userId) => {
          if (userId) {
            // Members: board-scoped so a trashed board's comments drop out
            // of sync for the 48h trash window along with the board itself
            // (board_id is trigger-denormalized and never null here).
