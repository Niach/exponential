...
// workspaceMembers.remove is the single membership-end path (kick AND
// self-leave). REV-8: removal must also delete the ex-member's
// issue_subscribers rows in that workspace inside the same transaction, so
// notification fan-out and the workspace-scoped issue-subscribers shape stop
...
function callerFor(userId: string) {
  return workspaceMembersRouter.createCaller({
...
describe(`workspaceMembers.remove — offboarding cleanup (REV-8)`, () => {
  it(`kick: deletes the membership row AND the ex-member's issue_subscribers rows`, async () => {
    selectQueue.push([
      { id: MEMBER_ID, userId: `user-b`, workspaceId: WS, role: `member` },
    ])
    ...
    expect(deletes[0]!.table).toBe(workspaceMembers)
  ...
  it(`still refuses to remove the last owner (guard survives the transaction refactor)`, async () => {
    selectQueue.push([
      { id: MEMBER_ID, userId: `user-b`, workspaceId: WS, role: `owner` },
    ])
    // The owners-of-workspace query finds a single owner — the target.
    selectQueue.push([
      { id: MEMBER_ID, userId: `user-b`, workspaceId: WS, role: `owner` },
    ])
