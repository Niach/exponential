...
// teams.delete must never delete the user's LAST personal team
...
describe(`teams.delete — last-personal-team guard (EXP-82)`, () => {
  it(`refuses to delete the user's only personal team`, async () => {
    // findOtherPersonalMembership: no membership besides the doomed team.
    selectQueue.push([])

    await expect(caller().delete({ teamId: WS })).rejects.toMatchObject({
  ...
  it(`deletes when another personal membership survives, reclaiming storage`, async () => {
    ...
    selectQueue.push([{ teamId: OTHER_WS }])
