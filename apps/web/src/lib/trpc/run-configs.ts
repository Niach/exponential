...
const nameSchema = z.string().trim().min(1).max(MAX_RUN_CONFIG_NAME)
...
const envSchema = z
  .record(
    ...
    z.string().max(MAX_ENV_VALUE_LENGTH)
  )
  .refine((env) => Object.keys(env).length <= MAX_ENV_ENTRIES, {
    message: `Too many environment variables`,
  })

const cwdSchema = z.string().trim().max(MAX_CWD_LENGTH).nullable()
