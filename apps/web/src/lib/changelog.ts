...
- **Styleguide**: a top bar switches between Views (the screenshots), Components (the live controls, grouped by kind) and Style (colours, shape, type, motion and the icon registry). Each component names which app surfaces still draw it by hand. The description editor's formatting rail and its link field are the same ghost buttons and text field as everywhere else. The composer card, the phone work bar, the work header, the context ring, the agent picker, the brand marks, the emoji picker, the lightbox and the results tiles now live in the shared component package with the rest, so the styleguide renders each one live, and the emoji picker's cells and empty line are the same button and empty state as everywhere else.`,
  },
  {
