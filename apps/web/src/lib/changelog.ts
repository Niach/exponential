...
- **Estimates**: issues now carry story points. Set them from the properties row on an issue, over the API and the agent tools; imports bring Linear's estimates with them.
...
- **More icons**: the board and action icon picker grows from 60 to 96 icons, with music, camera, gift, store, truck, map, mountain, flame, crown, brain, fingerprint, wallet, smartphone and more.`,
  },
  {
    ...
    body: `- **One picker**: assignee, board, labels, actions, branches, MCP servers, automation filters and the batch issue picker share a single searchable picker. A picked row shows a trailing check; in a multi-select every row carries the same filled or empty circle the iOS and Android sheets draw, and "Unassign" or "None" is a real row instead of a hidden placeholder value. The issue row's right-click submenus and the bulk bar draw the same rows: the bulk bar now marks what the whole selection already has, and a label on only some of the selected issues shows a circled minus. The status and priority menus, the settings rows (delivery, timezone, transport, the automation schedule), the device, model, repository and account words under the composer, the issue search and the GitHub repository list are the same picker too, so the status you have is checked in its menu on the desktop as well.
...
- **Styleguide**: a top bar switches between Views (the screenshots), Components (the live controls, grouped by kind) and Style (colours, shape, type, motion and the icon registry). Each component names which app surfaces still draw it by hand. The description editor's formatting rail and its link field are the same ghost buttons and text field as everywhere else. The composer card, the phone work bar, the work header, the context ring, the agent picker, the brand marks, the emoji picker, the lightbox and the results tiles now live in the shared component package with the rest, so the styleguide renders each one live, and the emoji picker's cells and empty line are the same button and empty state as everywhere else. The small shapes followed: count badges, the fold headers in a run's conversation, the dashed "describe one" nudge on the actions page, the floating circles in the phone bars, the compact sidebar rows, posted images and the "Show more" links are one component each now, so they look and behave identically wherever they turn up.
- **Desktop**: the IDE draws the same set. Text fields wear the field's 12px corner instead of the row's 10. The diff pane's file filter, the emoji, issue and branch searches, the repository lookup and the search dialog are one search field with the glyph inside and a clear circle that appears once you type. The label and board pickers and the MCP server picker are one searchable picker with the same marks as the web and the phones: a trailing check for a single pick, the filled or empty circle for a multi. The run conversation's tool groups, Exponential calls, subagent lanes and workflow agents fold on one header that brightens under the pointer; "Show more" and the edited-files footer are one text button; the rail's Drafts entry carries its count in the same small capsule the web shows. The @, # and : menus in the editors and the composer, and the / menu over the run composer, are one menu now, and it opens above the caret when the window runs out of room below. A run that is working ripples its green dot like the web does; the loading bars on Drafts, Reviews and a support ticket, the bulk-select boxes in the issue list and the rail, and the checklist boxes in a description are the same shapes as on the web.`,
  },
  {
...
- **Chats named by the agent**: a chat run shows the title Claude Code or codex gave the conversation instead of "Chat", in every session list, tab and review row on the web, the desktop app, iOS and Android. Renaming it in the agent renames it here.`,
  },
  {
    ...
    body: `- **One file card**: a review, a run's changes and a transcript all draw the same card now, with the same header, the same counts and the same way to open the lines it hides. Cards start open, so a diff reads top to bottom without a click.
...
- **Reviews polish**: a review's header says which issue it is, how big the diff is and what the pull request's state is, with reject, Merge PR and the GitHub link beside it. Batch rows in the review queue no longer lose their first column. On phones the floating bottom bar is now the same on web, iOS and Android: one geometry, no drop shadow, and Merge PR is one white pill centred between the circles, on a review and on a run's changes alike.`,
  },
  {
    ...
    body: `- **One usage overlay**: the context ring opens the same view on web, desktop, iOS and Android: the run's own account at the top with its three rate-limit windows, the context window below, and every other account on that machine as a compact row with a switch button beside it instead of across it.
...
- **Usage page**: every machine and account you own, grouped by agent with its rate-limit windows, with a Refresh that respects the provider's limits.`,
  },
  {
    ...
    body: `- **Tool groups that say what happened**: a collapsed run of tool calls reads "Ran 4 commands · edited 2 files · 1 failed" instead of "12 tool calls", with failures last, on web, desktop, iOS and Android.
...
- **Terminal-mode coding retired**: every coding session runs on the session page. The Start in terminal switch and the terminal-tab fallback are gone; plain terminal tabs and agent sign-in tabs stay. Where a picker used to say a start would open in a terminal, it now says the agent is not ready on that machine and blocks the start until you run the doctor there.`,
  },
  {
    ...
    body: `- **Issue preview cards**: hovering a #EXP-12 pill in a description, comment, timeline, relation row or steering feed on web and desktop opens a card with the issue's identifier, assignee, title, status, priority and labels. On iOS and Android the chips in the steering feed open the issue on tap.
...
- **Android**: search results show their group counts.`,
  },
  {
    ...
    body: `- **Slash commands**: typing \`/\` in a steering chat offers \`/compact\` and \`/clear\`. They run on claude, codex and pi, on web, iOS, Android and the desktop viewer.
...
- **Fresh conversation, with a warning**: \`/clear\` asks for confirmation first, because it discards everything the agent knows so far. Files in the worktree are kept.
...
- **Mobile**: tap any cell to edit its text. To remove a table, long-press a cell on iOS or Android, or use the keyboard bar's Delete table on the mobile web; row and column changes stay on web and desktop.`,
  },
  {
...
- **Terminal core**: the emulator moved from alacritty_terminal to rio-vt, the VT core behind the Rio terminal, which is what brings the graphics support. Key handling, mouse reporting, selection and live steering behave exactly as before.`,
  },
  {
    ...
    summary: `Live coding sessions on web, iOS and Android now ride out network changes and app switches, the CLI daemon heals itself when it falls behind, and the feedback widget can hide its launcher on demand.`,
    body: `- **Sessions that stay up**: the web, iOS and Android session views reconnect on their own after a network change, a backgrounded app or a dropped socket, with capped, jittered retries instead of a spinner that never ends.
- **No stranded sessions**: a desktop that is told it is too old still ends its coding sessions cleanly instead of leaving them marked as running, and the CLI daemon updates itself immediately when it hits that gate (\`exponential update\` now restarts a running daemon too).
- **Widget**: a new \`setLauncherHidden()\` call hides the launcher while your own UI covers its corner, the edge tab is slimmer, and the docs cover the whole runtime API.
- **Polish**: a calmer Actions list on every client, refreshed product demos in the docs, and the desktop notifications that shipped alongside (see the entry below).`,
  },
  {
...
- **Quiet when it should be**: nothing fires for the history that syncs at first sign-in, what arrived while the app was closed shows up once as a single bundle, a burst of related events collapses into one notification, and an issue you are already looking at stays silent.
...
- **Same chat as always**: pick a machine, pick a repository, type what you want, and the session opens as soon as your machine takes it.`,
  },
  {
    ...
    body: `- **One toggle**: open a machine's settings from Agents and flip "Default device". A star marks it in the machines list, and only one of your machines can hold it, so turning it on somewhere else moves it.
...
- **Web composer**: images can now be dragged onto the steering box, and attaching twice in a row no longer loses the first image.`,
  },
  {
    ...
    summary: `Opening or merging a pull request on GitHub itself is now credited to your account here, so you stop hearing about your own work.`,
    body: `- **GitHub actions get a name**: a pull request opened or merged on github.com used to arrive as "a pull request was opened", with nobody excluded, so it reached the very person who had just done it. Your GitHub account is now linked when you connect GitHub in team settings, and those events are credited to you and skipped for you.
