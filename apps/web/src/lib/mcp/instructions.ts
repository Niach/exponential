...
export const MCP_SERVER_INSTRUCTIONS = `Exponential is this team's issue tracker: issues on boards, with comments, labels and the pull requests that close them. Inside a coding session the flow is exponential_issues_get, exponential_comments_list, implement, commit and push, exponential_pr_open, and exponential_sessions_end last. Never set an issue to 'in_review' yourself; the PR tools move issues for you. Search for exponential_* tools for boards, labels, members, attachments, notifications, actions, automations, repositories and teams.

exponential_sessions_end only works inside a session started by the Exponential launcher, and it is how a run reports back: a one-paragraph summary plus outcome 'done' (the PR is open or the work is complete), 'blocked' (you stopped and a human is needed) or 'no_changes' (nothing needed changing). Leave the worktree clean before you call it. Merging your own pull request does not end your session; only exponential_sessions_end does. exponential_pr_open takes 'issueId' for one issue, 'issueIds' plus 'head' for one combined PR over several, or 'repositoryId' plus 'head' for a chore PR with no issue at all. exponential_pr_merge mirrors that: 'issueId'/'issueIds', or 'repositoryId' plus 'prNumber'. If a merge is refused because the base is stale, call exponential_pr_retarget, rebase onto the new base, force-push with --force-with-lease, then merge again.`
