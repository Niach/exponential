...
export const COMPONENTS: readonly ComponentSpec[] = [
  ...
  },
  {
    ...
    title: `Dock header`,
    kind: `Controls`,
    blurb: `The bottom dock's chrome, in two parts. A 28px header row tops the OPEN panel and carries only the window controls, right-aligned: open in a window, the collapsed-form switch, then collapse — 20px ghost glyph buttons, nothing else, because a header that also held tabs would be a second tab strip. Below it the strip holds the rich tabs and the add button; the dock itself is an opaque popover under a card hairline, with the bottom corners rounded to the panel it sits in. COLLAPSED, the dock is either that strip alone or the BUBBLE (EXP-742): the glass card recipe — radius 16, card hairline — on the opaque popover fill, floating 10 from the panel's bottom-right corner, padding 4 with 6 on the left, carrying an aggregate 6px status dot and the mono entry count, the same rich tabs (at most three, the rest fold into a muted "+N"), and a chevron-up tool. The whole card opens the dock; a chip opens it on that tab. Which form a collapse lands on is a per-device pick.`,
    status: {
      web: ok(
        `AgentDock`,
      ...
      desktop: ok(
        ...
        `the bubble is render_bubble in the same file, hosted by Shell over the cutout panel`
      ),
    ...
    render: () =>
      [
        ...
        `</div>`,
        `<div class="cmp-inline">`,
        `<div class="cmp-dock-bubble">`,
        `<span class="dot"></span>`,
        `<span class="amount">4</span>`,
        richTab({ glyph: svgTerminal, title: `zsh`, id: `1`, active: true }),
        richTab({ dot: true, title: `Fix the merge queue`, id: `APP-14` }),
        `<span class="caption">+2</span>`,
        `<button class="tool" type="button">${svgChevronUp}</button>`,
        `</div>`,
        `</div>`,
        `</div>`,
      ].join(``),
