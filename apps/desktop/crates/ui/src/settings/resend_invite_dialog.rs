...
/// description (web `resendTarget.name`).
pub(super) fn open(
    window: &mut Window,
    cx: &mut App,
    team_id: String,
    ...
    let spec = DialogSpec::new("Resend invite", size(px(420.), px(260.)))
        .resizable(size(px(360.), px(220.)));
    native_dialog::open_dialog_window(window, cx, spec, move |window, cx| {
        let view =
            cx.new(|cx| ResendInviteDialogView::new(team_id, display_name, target, window, cx));
        let busy = view.clone();
