...
impl Render for MentionInput {
    fn render(&mut self, window: &mut Window, cx: &mut gpui::Context<Self>) -> impl IntoElement {
        ...
        div()
            ...
            .relative()
            .w_full()
            // EXP-491: a FLEX column, not gpui's default block — the same
            // pinned-taffy quirk the editor documents (EXP-436): a
            // percent-width child under a display-BLOCK parent resolves
            // fit-content on intrinsic passes, collapsing the composer to a
            // ~20px pill at certain panel widths once a comment row shares
            // the column. Flex cross-axis stretch sizes the Input
            // definitively.
            .flex()
            .flex_col()
            .items_stretch()
