...
/// EXP-993 made it the FIRST repo whatever the count). A repo-less chat was
/// the old default with several connected repos, which meant the common case
/// — a team with two repos — started every chat in a scratch dir and made the
/// agent ask. The picker offers the other repos; there is no "no repository"
/// entry any more, so the seed has to be a real one.
