...
mod tests {
    ...
    fn a_retired_start_in_terminal_key_is_dropped_on_save() {
        ...
        fs::write(
            &path,
            r#"{"startInTerminal":true,"claudeKeepAlive":true,"launchDefaultsSync":{"desk-1":{"dirty":true}}}"#,
        )
        .unwrap();
        let settings = Settings::load(&path);
        settings.save(&path).unwrap();
        ...
        assert!(root.get("claudeKeepAlive").is_none(), "EXP-909: retired with the toggle");
