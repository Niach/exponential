...
impl Block {
    fn pasted_image_source_from_clipboard(item: &ClipboardItem) -> Option<PastedImageSource> {
        item.entries().iter().find_map(|entry| match entry {
            ...
            // gpui at the zed pin also carries file paths on the clipboard;
            // local-path image paste is handled via the text branch instead.
    ...
    pub(crate) fn on_newline(&mut self, _: &Newline, window: &mut Window, cx: &mut Context<Self>) {
        ...
        // EXP-261 vendoring: the `$$`-then-Enter display-math conversion was
        // removed together with LaTeX rendering — `$$` lines stay ordinary
        // paragraphs.
