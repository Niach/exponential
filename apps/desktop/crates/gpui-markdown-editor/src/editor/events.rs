...
mod tests {
    ...
    async fn plain_multiline_paste_with_leading_inline_html_splits_physical_lines(
        ...
        editor.update(cx, |editor, cx| {
            ...
            let visible = editor.document.visible_blocks();
            assert_eq!(visible.len(), 3);
            // EXP-261 vendoring: sup/sub have no GFM representation and stay
            // literal text.
            assert_eq!(visible[0].entity.read(cx).display_text(), "<sub>2</sub>");
            assert_eq!(visible[1].entity.read(cx).display_text(), "<sup>n</sup>");
