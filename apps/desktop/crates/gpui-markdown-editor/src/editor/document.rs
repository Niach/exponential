...
fn is_display_math_start(line: &str) -> bool {
    // EXP-261 vendoring: LaTeX rendering was removed, so `$$` lines are never
    // classified as display-math blocks — they fall through to plain
    // paragraphs / raw markdown and round-trip verbatim (matching the GFM
    // interchange contract, where `$$` has no special meaning).
    let _ = line;
    false
}
...
fn collect_fenced_code_block(
    ...
    // EXP-261 vendoring: Mermaid rendering was removed — ```mermaid fences
    // parse as ordinary fenced code blocks and round-trip verbatim.
...
mod tests {
    ...
    #[gpui::test]
    async fn display_math_lines_stay_raw_paragraphs_exp261(cx: &mut TestAppContext) {
        // EXP-261 vendoring: `$$` blocks are never classified as math.
        let markdown = "before\n\n$$\nx+1\n$$\n\nafter".to_string();
        let editor = cx.new(|cx| Editor::from_markdown(cx, markdown.clone(), None));
        editor.update(cx, |editor, cx| {
            let visible = editor.document.visible_blocks();
            assert!(
                visible
                    .iter()
                    .all(|block| block.entity.read(cx).kind() != BlockKind::MathBlock)
            );
        });
        editor.update(cx, |editor, cx| {
            assert_eq!(editor.current_document_source(cx), markdown);
        });
    }

    #[gpui::test]
    async fn mermaid_fence_parses_as_plain_code_block_exp261(cx: &mut TestAppContext) {
        // EXP-261 vendoring: ```mermaid fences are ordinary code blocks.
        ...
        editor.update(cx, |editor, cx| {
            ...
            assert_ne!(block.kind(), BlockKind::MermaidBlock);
        });
        editor.update(cx, |editor, cx| {
            assert_eq!(editor.current_document_source(cx), markdown);
        });
