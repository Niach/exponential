...
//
// Multi-account: each signed-in account runs its own set of 21 shape Tasks in
...
public final class SyncManager: @unchecked Sendable {
    ...
    // EXP-656: the URLSession each account's 21 shapes share, so a park can
    ...
    // 21 uncancellable shape Tasks (duplicate long-polls racing the wipe).
    ...
    /// row + saved offset, then relaunch so all 21 shapes refetch from scratch.
    ...
    public func restartPipeline(accountId: String, reason: String = "membership change") async {
        ...
        // the pipeline over this one and orphan 21 uncancellable shape Tasks.
    ...
    /// 21 long-polls are torn down instead of being carried into suspension on
    ...
    private func relaunchAllNow(reason: String) {
        for accountId in auth.accounts.filter({ $0.token != nil }).map(\.id) {
            ...
            // launch its own — stepping in would orphan 21 uncancellable tasks.
    ...
    private func launchPipeline(accountId: String, pool: DatabasePool) {
        logger.info("Launching live shape sync (21 shapes) for account \(accountId, privacy: .public)")
        ...
        SyncDebug.shared.log("[pipeline] launched 21 shapes")
        ...
        // ONE session for all 21 shapes of this account (EXP-304). Per shape it
        // meant 21 separate connections, so every launch fired 21 simultaneous
        ...
        // EXP-736: issue relations — a board-scoped child of the SOURCE issue,
        // the 20th shape.
        tasks.append(makeShapeTask(
            name: "issue-relations", path: "/api/shapes/issue-relations", table: "issue_relations",
            type: IssueRelationEntity.self, accountId: accountId, pool: pool, baseUrl: baseUrl, token: token,
            session: session
        ))

        // EXP-778: personal pins — static per user (`user_id = me`), the 21st
        // shape. Not team-scoped: the reader filters on team_id locally.
        tasks.append(makeShapeTask(
            name: "pins", path: "/api/shapes/pins", table: "pins",
            type: PinEntity.self, accountId: accountId, pool: pool, baseUrl: baseUrl, token: token,
            session: session
        ))
